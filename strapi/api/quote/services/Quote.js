...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Quote
      .findOne(_.pick(params, _.keys(Quote.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Quote.create(data);

    // Create relational data and return the entry.
    return Quote.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Quote.associations.map(a => a.alias));
    const data = _.omit(values, Quote.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Quote.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Quote.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Quote.attributes).reduce((acc, curr) => {
      switch (Quote.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
